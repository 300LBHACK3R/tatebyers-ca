...
function BrandCard({ collection }: { collection: LinkCollection }) {
  ...
  const isSocialLinksCard = collection.slug === "social-links";
  const primaryButtonLabel = getPrimaryButtonLabel(collection);

  const buttonLinks = isSocialLinksCard
    ? collection.links
    : href
  ...
  return (
    <div className="electric-card rounded-[2rem] p-[1px]">
      <article className="flex h-full flex-col rounded-[1.95rem] border border-white/8 bg-gradient-to-br from-white via-[#fffaf0] to-[#f0f0f0] p-5 text-[#101010] shadow-[0_30px_70px_rgba(0,0,0,0.36)] sm:p-6">
        ...
        {href || buttonLinks.length > 0 ? (
          <div className="mt-6 flex flex-wrap gap-2">
            {!isSocialLinksCard && href ? (
              <a
                href={href}
                {...getLinkProps(href)}
            ...
            {buttonLinks.slice(0, 8).map((link) => (
              <a
                key={`${collection.slug}-${link.title}`}
                href={link.href}
                {...getLinkProps(link.href)}
                className="inline-flex items-center gap-2 rounded-2xl bg-[#b70f1b] px-4 py-3 text-sm font-black text-white shadow-[0_14px_30px_rgba(183,15,27,0.28)] transition hover:-translate-y-0.5 hover:bg-[#111111]"
              >
                {link.title}
                <ExternalIcon />
              </a>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <section className="mt-12">
            <SectionLabel
              ...
              body="A clean place to find my active businesses, creative projects, and public social links."
            />
