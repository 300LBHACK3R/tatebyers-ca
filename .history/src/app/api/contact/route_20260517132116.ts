...
export async function POST(request: Request) {
  try {
    ...
    const contactFromEmail =
      getEnvValue("CONTACT_FROM_EMAIL") ?? "TateByers.ca <onboarding@resend.dev>";
    ...
    const subject =
      normalizeString(payload.subject, MAX_SUBJECT_LENGTH) || "New TateByers.ca message";
    ...
    const { data, error } = await resend.emails.send({
      ...
      text: buildPlainTextEmail({ name, email, subject, message }),
      replyTo: email,
    });
